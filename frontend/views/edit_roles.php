...
if (isset($_GET['id'])) {
    ...
    $url = 'http://127.0.0.1:8000/api/rols/' . $id_rol;
    $ch = curl_init($url);
...
<body class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="w-11/12 sm:w-5/6 md:w-2/3 lg:w-1/2 xl:w-1/3 p-3 bg-white shadow-md rounded-lg">
        <div class="flex justify-between">
            <h1 class="p-3 text-2xl">Editar Rol</h1>
            <a href="javascript:history.back()" class="text-gray-600 text-lg"><i class="fas fa-times mr-1"></i></a>
        </div>
        <p class="text-blue-500"> Selecciona un nuevo Rol *(Requerido) </p>


        <form method="POST" action="http://127.0.0.1:8000/api/rols/<?php echo $id_rol; ?>">
            <input type="hidden" name="_method" value="PUT">
            
            <div class="p-1">
                <label for="exampleInputEmail1" class="form-label">Id *</label>
                <input type="text" class="form-control p-1" name="id" value="<?= $usuario['id']; ?>">
            </div>
